...
//int vmin = 72, vmax = 255, smin = 190; //Gazebo/Red
...
int main( int argc, char** argv )
{
    ...
    image_transport::Subscriber sub_img1 = img_trans.subscribe("/camera/color/image_raw",1,imageCallback);
    //image_transport::Subscriber sub_img1 = img_trans.subscribe("/usb_camera/color/image_raw",1,imageCallback);
    sleep(1);
    ros::spinOnce();
    ...
    //ros::Subscriber sub = handle1.subscribe<sensor_msgs::PointCloud2>("/usb_camera/depth/points", 1, boost::bind(ptCloudcallBack, _1, boost::ref(ptcloud),boost::ref(flag_ptcloud)));
    sleep(1);
    ros::spinOnce();
...
//                cout << "Object area from cam shift " << trackBox.size.area() << endl;
        ...
        double m[2];
        m[0] = (iLastX-320)/(658.2874220024545/640);    //(u-u0)/px
        m[1] = (iLastY-240)/(658.2874220024545/480);    //(v-v0)/py50
