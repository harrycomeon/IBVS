...
int main (int argc, char** argv)
{
    ...
    ros::Publisher StatePub = nh.advertise<geometry_msgs::Twist>("/states_pid",1);

    ros::Publisher ActionPub = nh.advertise<geometry_msgs::Wrench>("/action_pid",1);

    //ros::Publisher StatePub = nh.advertise<ur5_vs::states>("/states_pid",1);
    

    //action pid subscriber
    ros::Subscriber actionpidsSub = nh.subscribe<geometry_msgs::Wrench>("/action_move_pid", 10, actionpidCallback);
    ...
    f1.open("/home/harry/catkin_Obj_det/src/ur5_vs/result/trackedfeatures.txt",std::ios_base::trunc);
    f2.open("/home/harry/catkin_Obj_det/src/ur5_vs/result/joint_velocities.txt",std::ios_base::trunc);
    f3.open("/home/harry/catkin_Obj_det/src/ur5_vs/result/error.txt",std::ios_base::trunc);
    f4.open("/home/harry/catkin_Obj_det/src/ur5_vs/result/camera_vel.txt",std::ios_base::trunc);
    ...
    while (ros::ok)
    {
        kpx=Kpu, kdx=Kdu, kix=Kiu; //PID gains
        kpy=Kpv, kdy=Kdv, kiy=Kiv; //PID gains
        ...
        //Way-2
        // computeImageJa cobian(m,depth_p,L);
    ...
    //     ur5_vs::states states;
    //     states.p0.data = Pterm.at<double>(0,0);
	// states.p1.data = Pterm.at<double>(1,0);
	// states.i0.data = Iterm.at<double>(0,0);
	// states.i0.data = Iterm.at<double>(1,0);
	// states.d0.data = Dterm.at<double>(0,0);
	// states.d0.data = Dterm.at<double>(1,0);

        geometry_msgs::Twist states;
        states.linear.x = Pterm.at<double>(0,0);
	states.linear.y = Pterm.at<double>(1,0);
	states.linear.z = Iterm.at<double>(0,0);
	states.angular.x = Iterm.at<double>(1,0);
	states.angular.y = Dterm.at<double>(0,0);
	states.angular.z = Dterm.at<double>(1,0);
        
        StatePub.publish(states);
        geometry_msgs::Wrench action;
